...
- calibrate_axis_weights: 축가중 a 그리드를 실현 IC 로 역산. 설계가 유일하게 a 만 캘리브레이션
  대상에서 제외(고정 (0.35,0.35,0.15,0.15))했던 공백을 메운다 — 그 값이 최적인지 데이터로 검증.
...
# 축가중 후보 그리드(합=1, 동일 4축 키). 첫 항목 = 현재 운영값(eval 시 baseline).
DEFAULT_AXIS_GRID: list[dict] = [
    dict(AXIS_WEIGHTS),                                                      # 현재 (0.35,0.35,0.15,0.15)
    {"news": 0.25, "pattern": 0.45, "anomaly": 0.15, "relationship": 0.15},  # pattern 강화
    {"news": 0.45, "pattern": 0.25, "anomaly": 0.15, "relationship": 0.15},  # news 강화
    {"news": 0.30, "pattern": 0.30, "anomaly": 0.25, "relationship": 0.15},  # anomaly 강화
    {"news": 0.30, "pattern": 0.30, "anomaly": 0.15, "relationship": 0.25},  # relationship 강화
    {"news": 0.25, "pattern": 0.25, "anomaly": 0.25, "relationship": 0.25},  # 균등
]
...
if __name__ == "__main__":
    ...
    print(f"→ best a = news {abest.get('news')}/pattern {abest.get('pattern')}/"
          f"anomaly {abest.get('anomaly')}/relationship {abest.get('relationship')} "
          f"(IC={abest.get('mean_ic'):+.4f})")
