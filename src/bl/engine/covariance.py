...
def _as_panel(returns) -> np.ndarray:
    ...
    # pandas DataFrame/Series는 to_numpy, 그 외는 np.asarray로 2D 변환
    arr = returns.to_numpy(dtype="float64") if hasattr(returns, "to_numpy") else np.asarray(returns, dtype="float64")
    if arr.ndim != 2:
