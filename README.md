...
- ✅ **구현·검증 완료** (전 파이프라인 오프라인 end-to-end, **93 테스트 통과**, 적대적 코드리뷰 4회 반영):
...
- 🔜 다음: 실데이터 라이브 검증 · 대시보드 디자인 적용(현재는 **와이어프레임**) · 운영화(스케줄·관측성)
- 격상 핵심 변경:
...
- **LLM**: Google Gemini 2.5 Flash-Lite (뉴스 감성 ∈[-1,1]) · 규칙기반 렉시콘 폴백(키 없을 때)
...
src/bl/
  ...
  refine/ enrich/  뉴스 dedup · Gemini/규칙 감성
...
tests/      93 passing      docs/  기획·설계·ADR      data/sample/  합성 데모(44KB)      site/  빌드된 대시보드
```
