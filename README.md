...
| 투자자 전망 (View, Q) | → | AI 3축 신호 앙상블 (news=Gemini 감성 · pattern=XGBoost 성장/이탈 · relationship=거래관계 강도) |
| 전망 불확실성 (Ω) | → | 데이터신뢰도(DRI)·모델 confidence·이상도(anomaly) ($\Omega \propto (1+\gamma\,\text{anomaly})/\text{DRI}^2$) |
| 최적 가중치 $w^{*}$ | → | **권장 영업자원 배분** |

> **뷰 $Q$는 3축으로 결합**한다: news(Gemini 감성) · pattern(XGBoost 성장−이탈) · relationship(거래관계 강도). 축 가중 $a$ = (news 0.412, pattern 0.412, relationship 0.176), 합=1 (과거 4축 0.35:0.35:0.15:0.15에서 anomaly 제외 후 비율보존 재정규화). **IsolationForest 이상도(anomaly)는 방향 뷰가 아니라 Ω 신뢰도 변조 요인**이다 — 이상(out-of-distribution)할수록 그 법인의 뷰를 앵커(prior)로 후퇴시킨다(DRI·confidence와 형제, 게인 $\gamma=2.0$). relationship은 계좌수·급여이체·주거래 등 거래관계 강도다.
>
...
이 프로젝트는 그 틀을 **B2B 예금유치 마케팅**에 이식한다 — 자산=법인 고객사, 시장 균형 비중=지갑(예금)규모 비중, 투자자 전망=AI 3축 신호 앙상블(이상도 anomaly는 Ω 신뢰도 변조), 최적 가중치=권장 영업자원 배분. 전망과 그 신뢰도를 **AI가 자동·재현 가능하게** 생성하고, BL은 그 신호를 **설명가능하게**(균형 vs 전망으로 분해) 결합한다.
...
- ✅ **구현·검증 완료** (전 파이프라인 오프라인 end-to-end, **123 테스트 통과**, 적대적 코드리뷰·다중에이전트 리뷰 반영):
...
→ **BL vs naive: lift +12.1%p · win-rate 100%(12/12 윈도우) · IC +0.32 · Precision@K +0.02.**

> 위 수치는 **E2(anomaly→Ω 신뢰도 변조)와 C3(앵커 λ를 Π 스케일 정규화 상수로 강등) 둘 다 적용된 현재 코드** 기준이다. 두 변경 모두 뷰를 prior(앵커)로 후퇴시키는 **규제(regularization)** 라서 결합 시 BL이 보수적으로 수렴한다 — 합성 positive-control에서 절대수익은 ≈0으로 줄지만(공격적 알파를 깎음) **naive의 −12.2% 손실은 피하고**(lift +12.1%p·win 100%), 액티브 틸트의 순위신호(IC +0.32)는 유지된다. 즉 가치가 "절대 초과수익"에서 "**하방 회피 + 신호 보존**"으로 이동했다. 결합 보수성(γ_anom=2 · λ_fix=0.25)이 과한지는 운영 정책 판단이며, 같은 백테스트로 재캘리브레이션 가능하다.

하이퍼파라미터도 같은 백테스트의 실현지표로 **역산**한다(`python -m bl.eval.calibrate`): τ∈{0.025,0.05,0.1}
은 전 구간 win-rate 100%로 안정이라 기존값 **τ=0.05 가 near-optimal**(절대 lift는 위 표 +0.12대; τ↑=공격/τ↓=보수 민감도는 [설계 §5.5](docs/design/03-bl-model-design.md)), 축가중은 **뷰 3축(news/pattern/relationship)** 그리드로, **anomaly의 Ω 변조 강도 $\gamma_{\text{anom}}$**(`calibrate_gamma_anom`)은 $\gamma\in\{0,1,2,4\}$ 그리드로 역산한다. 결합 코드(E2+C3)에서도 **$\gamma=0$(lift +0.149)이 lift 최대** — *anomaly에 방향 알파가 없다는 본 설계전환(E2: anomaly를 4번째 방향 뷰에서 Ω 신뢰도 요인으로 이전)을 데이터가 확인*한다. 기본 **$\gamma=2$**(lift +0.120)는 실데이터에서 일회성 이상치(M&A 대금·세금환급 등)의 뷰를 prior(앵커)로 후퇴시키는 보수적 사전이며(win 100%·IC 유지, 단 lift·IR는 γ=0 대비 하락 — 일회성 이상치 방어 비용), 실데이터 도착 시 동일 도구로 재캘리브레이션한다.
...
>
> ⚠️ **번영 추종 ≠ 영업효과(채점 기준 명시)**: 현 백테스트는 **raw 실현 잔액수익(번영)** 으로 채점하므로
> 위 alpha/lift 는 *영업 처치효과(uplift)가 아니라 '번영 추종' 지표*다(C1 미해결 — 현 뷰는 *번영 예측*).
> **uplift 채점(처치 레이어)으로 전환한 뒤 재해석**해야 하며, 그 전까지 공격성 상향은 보류한다.
...
`calibrate_gamma_anom`. 합성에선 **세 손잡이 모두 공격적일수록 raw lift/IC/IR 단조 증가**(스펙트럼 표 [REPORT.md §10](REPORT.md))인데, 이는 *번영 추종*이라 **굿하트 위험** — 그래서 **기본은 보수값 고정**(τ=0.05·λ_fix=0.25·γ_anom=2.0)하고, 운영값은 **처치 레이어(uplift 채점) 머지 후 KB 실데이터 재캘리브레이션으로 확정**한다(지금 한 점 고정 금지).
