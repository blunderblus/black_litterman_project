...
- ✅ **구현·검증 완료** (전 파이프라인 오프라인 end-to-end, **120 테스트 통과**, 적대적 코드리뷰·다중에이전트 리뷰 반영):
...
→ **BL vs naive: lift +19.4%p · win-rate 100%(12/12 윈도우) · IC +0.32 · Precision@K +0.12.**

하이퍼파라미터도 같은 백테스트의 실현지표로 **역산**한다(`python -m bl.eval.calibrate`): τ∈{0.025,0.05,0.1}
은 lift 0.19대로 전 구간 안정(win-rate 100%)이라 기존값 **τ=0.05 가 near-optimal**, 축가중은 **뷰 3축(news/pattern/relationship)** 그리드로, **anomaly의 Ω 변조 강도 $\gamma_{\text{anom}}$**(`calibrate_gamma_anom`)은 $\gamma\in\{0,1,2,4\}$ 그리드로 역산한다. 합성에선 **$\gamma=0$(lift +0.197)이 lift 최대** — *anomaly에 방향 알파가 없다는 본 설계전환(E2: anomaly를 4번째 방향 뷰에서 Ω 신뢰도 요인으로 이전)을 데이터가 확인*한다. 기본 **$\gamma=2$**는 실데이터에서 일회성 이상치(M&A 대금·세금환급 등)의 뷰를 prior(앵커)로 후퇴시키는 보수적 사전이며(lift/IR/win 보존, IC 변화 비유의 t≈−1.2), 실데이터 도착 시 동일 도구로 재캘리브레이션한다.
