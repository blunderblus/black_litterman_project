...
| 투자자 전망(View, $Q$) | AI 뷰 레지스트리 블록스택 (현 news·pattern 2뷰) | XGBoost(성장−이탈)·Gemini(감성); IsolationForest 이상탐지는 뷰가 아니라 $\Omega$ 신뢰도 요인 |
...
| FR-05.4 | $P$ 행렬을 절대뷰/상대뷰로 명시 구성하고 $Q$ 와 정합. 뷰 결합은 손가중이 아니라 **뷰 레지스트리 블록스택**(현 news·pattern 2뷰)으로 수행: $P=[I;I]$ 블록스택(KN×N), $Q$=뷰별 단위정합 블록 쌓기(가중합 폐기). 뷰 상관은 $\Omega$ off-diagonal(뷰상관 $R_{\text{view}}$, 표준화 신호상관 프록시)이 담당(과거 축가중 $a=(0.412,0.412,0.176)$ 손가중 결합은 E3a로 폐기). relationship은 방향 예측이 아니라 현재상태=이동성이라 뷰에서 제외하고 $\Sigma$ 이동성 슬롯으로 예약(E1b 대기). anomaly는 뷰가 아니라 $\Omega$ 신뢰도 변조 요인(FR-05.5). |
...
- [ ] 뷰 레지스트리 블록스택(현 news·pattern 2뷰)이 $P=[I;I]$·$Q$ 블록 쌓기로 구성되고, 뷰 상관은 $\Omega$ off-diagonal($R_{\text{view}}$)로 반영됨을 검증한다(과거 축가중 $(0.412,0.412,0.176)$ 손가중 결합은 E3a로 폐기). relationship은 뷰에서 제외되어 $\Sigma$ 이동성 슬롯으로 예약됨을 확인한다. anomaly는 뷰가 아니라 $\Omega$ 신뢰도 변조 요인 $(1+\gamma_{\text{anom}}\,a),\ \gamma_{\text{anom}}=2.0$ 으로 적용됨을 검증한다.
...
**확정 파라미터(BL설계 §11 단일 출처):** $w_{\max}=0.10$, $\tau=0.05$(민감도 $\{0.025,0.05,0.1\}$), $\lambda$ 클립 $[1,5]$, $\lambda_{\text{floor}}=10^{-8}\cdot\mathrm{tr}\Sigma/N$, $\kappa_{\max}=10^{6}$, 뷰 레지스트리 블록스택(현 news·pattern 2뷰; 손가중 축가중 $(0.412,0.412,0.176)$은 E3a로 폐기, 뷰 결합은 $\Omega$ off-diagonal $R_{\text{view}}$), anomaly $\Omega$ 게인 $\gamma_{\text{anom}}=2.0$.
...
| ID | 우선순위 | 요구사항 |
|---|---|---|
| **FR-07** | **Should** | 기업별 스코어카드를 생성한다: 모델 의견(뷰 레지스트리(현 news·pattern 2뷰) + anomaly는 $\Omega$ 신뢰도 변조 요인, 표시 라벨 "이상도(신뢰도 차감)"; relationship은 방향 뷰가 아니라 $\Sigma$ 이동성 슬롯 예약), DRI, 사후수익 $E[R]$, 권장가중 $w^{*}$, 티어/섹터, 자사 보유 여부. |

**수용기준(AC):**
- [ ] 한 화면에서 한 기업의 모델 의견 신호(뷰 레지스트리(현 news·pattern 2뷰) + anomaly 신뢰도 차감)·신뢰도·권장액션 입력값이 보인다.
