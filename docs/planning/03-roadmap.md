...
| `04_track_C.ipynb` | 뉴스 수집 | — (격상판 제외) | — | 격상판 제외: BigKinds 폐쇄적 API |
| `05_유사뉴스_정제.ipynb` | 뉴스 중복 제거 | `bl/refine/news_dedup.py` | `bl.refine.news_dedup` | Kiwi 키워드·결정적 뉴스 dedup |
...
- WBS-D4: refine(뉴스 dedup) + enrich(Gemini 감성, confidence 캘리브레이션).
...
- WBS-B3: P 행렬 명시 구성(절대뷰=블록스택 $P=[I;I]$, 상대뷰)과 Q 정합(뷰별 단위정합 블록 쌓기, $Q\in\mathbb{R}^{KN}$), **뷰 레지스트리 블록스택**(현 news·pattern 2뷰) 매핑. 뷰 결합은 손가중이 아니라 **$\Omega$ off-diagonal(뷰상관 $R_{\text{view}}$, 표준화 신호상관 프록시)**이 수행한다(손가중 $a$=(0.412, 0.412, 0.176)은 E3a로 폐기). relationship은 방향 예측이 아니라 현재상태=이동성이라 뷰에서 제외하고 **$\Sigma$ 이동성 슬롯으로 예약**(실현 데이터 E1b 대기). anomaly는 뷰가 아니라 §B4의 $\Omega$ 신뢰도 변조 요인으로 분리([03 §5.1·§5.4](../design/03-bl-model-design.md)).
- WBS-B4: Ω 구성 — $\Omega \propto 1/\text{DRI}^2$ + 모델 confidence + **anomaly 신뢰도 변조** $(1+\gamma_{\text{anom}}\,a)$($a$=`anomaly_score_raw`$\in[0,1]$, $\gamma_{\text{anom}}=2.0$; 이상↑→Ω↑→해당 법인 뷰가 앵커로 후퇴), Q·Ω·τΣ **단위 통일**([03 §5.4](../design/03-bl-model-design.md)).
...
| R-22 | 외부 API(DART/ECOS/Naver) 스키마 변동 | 어댑터 계층 + pandera 검증으로 조기 탐지 | P3 | DE |
