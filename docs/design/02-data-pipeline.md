...
BL 최적화 직전 입력 묶음. 자산별 단위정합 상태로 보관(실제 산출 `bl_input_details_T1_202510.csv` 스키마 반영). **뷰 3축 앙상블($Q_{final}$)의 원천 신호와 Ω 요인을 한 행에 갖춘다**(news/pattern/relationship + anomaly는 Ω 요인).
...
| `Q_final` | double | 뷰값 $Q$ | 뷰 3축 앙상블(news/pattern/relationship, 단위정합, [03 BL §5.2](./03-bl-model-design.md)) |
...
> **뷰 3축→$Q_{final}$ 매핑(컬럼명 고정, [03 BL §5.1](./03-bl-model-design.md)과 동일)**: news=`gemini_score`, pattern=`prob_growth_raw − prob_churn_raw`, relationship=`relationship_score`. anomaly(`anomaly_score_if`)는 방향 뷰가 아니라 $\Omega$ 신뢰도 변조 요인(§5.4). 세 축 + anomaly Ω 요인 모두 `bl_input_data`에서 재현 가능해야 BL 입력 계약이 충족된다.
>
...
| 뷰 3축 완전성 | `bl_input_data`에 news/pattern/relationship 3축 원천 + Ω 요인(`anomaly_score_if`) 존재·non-null | bl_inputs |
...
sequenceDiagram
  ...
  M->>B: 뷰(P·Q·Ω, 3축+anomaly Ω요인)·Σ(FULL)·w_mkt → bl_input_data(+사이드카) → bl_result(E[R], w*)
...
| 12 | 검증 게이트 부재 | 결함 무점검 통과 | pandera류 스키마·무결성·누수·뷰 3축완전성 게이트(위반=빌드 실패) | §7 |
