...
> 컬럼 정리: 과거안의 `risk_score`는 산출식·소비처가 부재한 고아 컬럼이어서 **스키마에서 제거**했다(다운스트림 news 축은 `sentiment_score`만 사용; [03 BL §5.1](./03-bl-model-design.md)). `event_type`은 위 ENUM 도메인으로 한정하고, enrich 단계가 본문 분류로 채운다(현재 ENUM 집합은 잠정안, §10).
...
| `anomaly_score_if` | DOUBLE | | **IsolationForest 이상점수(canonical)**. 다운스트림 표시명 `anomaly_score_raw`의 원천(§3.4) → Ω 신뢰도 변조 요인(03 BL §5.4), 방향 뷰 아님 |
...
BL 최적화 직전 입력 묶음. 자산별 단위정합 상태로 보관(실제 산출 `bl_input_details_T1_202510.csv` 스키마 반영). **뷰 레지스트리 블록스택($Q$)의 원천 신호와 Ω 요인을 한 행에 갖춘다**(현 방향 뷰 2개=news·pattern; relationship은 $\Sigma$ 이동성 슬롯 예약, anomaly는 Ω 신뢰도 요인 — E3a).
...
| `Q_final` | double | 뷰값 $Q$ | 뷰 레지스트리 블록스택(현 news·pattern 2뷰의 단위정합 블록 쌓기; relationship→$\Sigma$ 예약, [03 BL §5.2](./03-bl-model-design.md)) |
...
| `anomaly_score_if` | double | $\Omega$ 원천 | **anomaly 이상도**(∈[0,1]; = `ML_PREDICTIONS.anomaly_score_if`) → $\Omega$ 신뢰도 변조 $(1+\gamma_{\text{anom}}a)$, [03 BL §5.4]; 방향 뷰 아님 |
| `relationship_score`,`trx_activity_score`,`account_count`,`has_payroll`,`is_main_bank` | double/int/bool | $\Sigma$ 예약/제약 | **$\Sigma$ 이동성 슬롯 예약**(relationship은 방향 뷰가 아니라 현재상태=이동성, E3a; 실현 데이터=E1b 대기)·제약 |
| `confidence` | double | $\Omega$ 보정 | 캘리브레이션 신뢰도 |

> **뷰 레지스트리(2뷰)→$Q$ 매핑(컬럼명 고정, [03 BL §5.1](./03-bl-model-design.md)과 동일)**: news=`gemini_score`, pattern=`prob_growth_raw − prob_churn_raw`. 뷰 결합은 손가중이 아니라 $P=[I;I]$ 블록스택 + $\Omega$ off-diagonal(뷰상관 $R_{\text{view}}$, 표준화 신호상관 프록시)이 수행한다(§5.2·§5.4, E3a). relationship(`relationship_score`)은 방향 뷰가 아니라 현재상태=이동성이라 뷰에서 제외하고 $\Sigma$ 이동성 슬롯으로 예약(E1b 대기), anomaly(`anomaly_score_if`)는 방향 뷰가 아니라 $\Omega$ 신뢰도 변조 요인(§5.4). 2뷰 원천 + relationship 예약 원천 + anomaly Ω 요인 모두 `bl_input_data`에서 재현 가능해야 BL 입력 계약이 충족된다.
>
...
| `prob_growth_raw`,`prob_churn_raw`,`anomaly_score_raw`,`news_sentiment`,`sentiment_confidence` | double | 신호 | **모델 신호 원천**(growth/churn/anomaly/news 표시, 별칭: §3.4) |
...
| 뷰 레지스트리(2뷰) 완전성 | `bl_input_data`에 news·pattern 2뷰 원천 + relationship($\Sigma$ 예약) 원천 + Ω 요인(`anomaly_score_if`) 존재·non-null | bl_inputs |
...
§3.2.2 실제 컬럼과 동기화하며, anomaly Ω 요인(`anomaly_score_if`)을 포함한다.
...
bl_input_schema = pa.DataFrameSchema({
    ...
    "anomaly_score_if":Column(float, nullable=False),            # anomaly 이상도 → Ω 요인
    "relationship_score": Column(float),                         # relationship → Σ 이동성 슬롯 예약(뷰 아님, E3a)
...
sequenceDiagram
  ...
  M->>B: 뷰(P·Q·Ω, 뷰 레지스트리 2뷰+anomaly Ω요인)·Σ(FULL)·w_mkt → bl_input_data(+사이드카) → bl_result(E[R], w*)
...
| 12 | 검증 게이트 부재 | 결함 무점검 통과 | pandera류 스키마·무결성·누수·뷰 레지스트리(2뷰) 완전성 게이트(위반=빌드 실패) | §7 |
| 13 | silent drop(미매핑/실패 폐기) | 추적 불가 | `DEAD_LETTER` + `UNKNOWN` 명시 보존 | §4.3, §7.3 |
| 14 | bare except | 결함 은폐 | 명시 예외 + 구조적 로깅 | §7.3 |
| 15 | anomaly 신호 BL 입력 누락·명칭 혼용 | $\Omega$ 신뢰도 변조 누락(이상 법인 뷰 과신), silent join 실패 | `anomaly_score_if`를 `bl_input_data`에 포함(Ω 요인, §5.4), 별칭 매핑표로 동치 고정 | §3.2.2, §3.4, §7.1 |
