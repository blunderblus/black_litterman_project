...
| $Q$ | 뷰 레지스트리 블록스택(현 news·pattern 2뷰) AI 신호. $P=[I;I]$ 블록스택($KN \times N$), $Q=$ 뷰별 단위정합 블록 쌓기 $Q_v=c_v\tilde s_v$. 뷰 결합은 손가중이 아니라 $\Omega$ off-diagonal(뷰상관 $R_{\text{view}}$)이 수행(축가중 $a=(0.412, 0.412, 0.176)$ 손가중 결합은 E3a로 폐기). relationship은 방향 뷰가 아니라 현재상태=이동성이라 $\Sigma$ 이동성 슬롯으로 예약(E1b 대기) | §5.1–5.2 |
