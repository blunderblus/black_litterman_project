...
| 입력 | 무엇이 채우는가 |
|---|---|
| 전망값 $Q$ | **4축 AI 앙상블** — news(Gemini 뉴스감성) · pattern(XGBoost 성장−이탈) · anomaly(IsolationForest 이상 × 자금흐름 방향) · relationship(거래관계 강도), 축가중 $a=(0.35, 0.35, 0.15, 0.15)$ |
| 신뢰도 $\Omega$ | **데이터신뢰도(DRI) + 모델 confidence** — $\Omega_{kk} \propto (P\tau\Sigma P^{\top})_{kk}\cdot 1/\mathrm{DRI}_i^2 \cdot (1-\mathrm{conf}_i)$ (단위정합 × 무차원 신뢰가중; 정식 정의 설계 §5.4) |
...
본 프로젝트의 매핑(표준화 신호 $\tilde s$ 에 회귀계수 $c$ 를 곱해 log-return 차원으로 사상, $Q = c\,\tilde s$; 4축 가중 결합)은 이 표준 패턴의 변형이며, **"B2B 예금유치"라는 비전통 자산군에 이식한 점**이 차별점이다. (한국 시장에서도 ML+BL 결합 선례가 있다 — 저위험 이상현상 연구, 2018.)

```mermaid
flowchart LR
  subgraph SIG["AI 신호 생성기 (블랙박스 격리 구역)"]
    G["Gemini 뉴스감성"]
    X["XGBoost 성장/이탈"]
    I["IsolationForest 이상"]
    R["거래관계 강도"]
  end
  SIG -->|"가중 결합 a=(0.35,0.35,0.15,0.15)"| Q["전망 Q_final"]
...
결합은 단방향(ML → BL 뷰 공급)이 아니다. BL은 거꾸로 **ML 신호를 정칙화**한다. 뷰가 없거나 신뢰도가 낮은(=$\Omega$ 큰) 자산은 사후가 자동으로 균형 $\Pi$ 로 수렴해 코너 해·극단 가중을 억제한다. 즉 BL은 노이즈 많은 ML 예측에 **"시장균형 사전"이라는 수축 앵커**를 씌워, raw ML 예측을 그대로 MVO에 넣을 때의 입력 민감성·코너 해를 완화한다. 본 프로젝트 표기로는 $\Pi = \lambda\Sigma w_{mkt}$(지갑규모 앵커)가 4축 신호의 노이즈를 흡수하는 prior로 작동한다. *4축 신호를 raw MVO가 아니라 BL을 거쳐 결합하는 이유가 바로 이것이다.*
...
에서 괄호 안은 **사전 기여 $(\tau\Sigma)^{-1}\Pi$** 와 **뷰 기여 $P^{\top}\Omega^{-1}Q$** 의 단순 합이다. 따라서 자산 $i$ 별로 *"사후가 균형에서 얼마나, 어느 방향으로 끌려갔는가"* 를 두 항의 상대 크기로 **정량 분해**할 수 있다. 정밀도(역공분산)로 가중된 합이므로, 뷰를 확신할수록($\Omega$ 작을수록) 뷰항 기여가 커진다. 이 분해는 [설계 §9.2 뷰 기여도/민감도](../design/03-bl-model-design.md)의 "기여 분해"와 정확히 일치하며, 4축 각각의 한계기여 $\partial E[R]/\partial Q_{axis}$ 까지 보고 가능하다.
...
$P$ 는 어느 자산에 전망을 걸었는지, $Q$ 는 그 전망값, $\Omega$ 는 그 불확실성을 *모두 명시적으로* 노출한다. 본 프로젝트에서 $Q_{final}$ 은 4축의 가중 결합이고 $\Omega$ 는 DRI·confidence에서 산정되므로, **각 뷰의 입력·가중·신뢰도가 데이터 계보(컬럼 출처)까지 단일 소스로 추적**된다([데이터 파이프라인](../design/02-data-pipeline.md)이 계보 권위). 추상적 파라미터를 운영자가 이해·감사 가능한 양으로 바꾼다는 점에서 Idzorek(2007)의 정신을 따른다.
...
각 화살표가 결정적·감사가능 변환이므로, 특정 법인의 "적극 유치" 처방을 *"어느 축 신호가 균형 대비 얼마나 끌어올렸는지"* 까지 역추적할 수 있다. [설계 §9.4 퇴화 진단](../design/03-bl-model-design.md)(방향-액션 일관성, 라벨 정합, weight_diff 스케일)이 이 traceability를 자동 검증한다.
...
- **AI가 주관성을 "제거"하지 않는다**: 축 가중·스케일 상수 $c$·DRI 가중·$c_{cal}$·$\eta$ 등은 여전히 설계 선택이다. AI는 주관을 *제거* 가 아니라 **체계화·재현가능화·문서화** 한다.
