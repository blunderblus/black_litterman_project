...
> **BL은 알파를 보장하지 않는다 — BL은 좋은 뷰를 일관되게 결합하는 회계장치(framework)일 뿐이다.** 본 프로젝트의 AI 앙상블(뷰 레지스트리 블록스택, 현 news·pattern 2뷰)은 바로 이 "$Q$ 를 체계적·재현가능하게 생성"하려는 시도다(→ [(4)편](./04-ai-augmentation-and-explainability.md)).
...
| 6 | garbage-in(뷰 주관성) | AI 앙상블(뷰 레지스트리 2뷰: news·pattern)이 인간 뷰의 주관·편향을 줄인 체계적·재현가능 뷰로 보완 | [§5.1](../design/03-bl-model-design.md) |
